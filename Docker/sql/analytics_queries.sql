...
/* ===================================================================================
  3) Top 10 companies by % price growth in Q1 2025
     ...
     (FIXED: compute first/last date per ticker, then join back for closes)
  =================================================================================== */
WITH
  per_ticker AS
  (
...
SELECT
  ...
  c.CompanyName,
  c.Sector,
  round(100 * (toFloat64(d2.Close) / toFloat64(d1.Close) - 1), 2) AS pct_growth
FROM per_ticker p
JOIN analytics.daily_stock_prices d1 ON d1.Ticker = p.Ticker AND d1.Date = p.min_d
JOIN analytics.daily_stock_prices d2 ON d2.Ticker = p.Ticker AND d2.Date = p.max_d
LEFT JOIN comp_q1 c ON c.Symbol = p.Ticker
WHERE d1.Close > 0
...
SELECT
  ...
  c.CompanyName                                AS CompanyName,
  c.Sector                                     AS Sector,
  round( avg( (d.High - d.Low) / nullIf((d.High + d.Low)/2, 0) ), 4) AS avg_rel_volatility
FROM analytics.daily_stock_prices AS d
ANY INNER JOIN universe AS u ON u.Symbol = d.Ticker
LEFT JOIN comp_q1 AS c        ON c.Symbol = d.Ticker
WHERE d.Date BETWEEN q1_start AND q1_end
GROUP BY Ticker, CompanyName, Sector
HAVING avg_rel_volatility IS NOT NULL
...
/* ==========================================================================================
  5) Sector composition of S&P companies (2000–2025)
